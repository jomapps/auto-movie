...
export default function ProgressIndicator({
  ...
  const defaultSteps: ProgressStep[] = [
    ...
    },
    {
      ...
      current: currentStep === 'characters',
    },
    {
      id: 'storyboard',
      title: 'Storyboarding',
    ...
    },
    {
      ...
      current: currentStep === 'assets',
    },
    {
      id: 'scenes',
      title: 'Scene Planning',
      completed: progress >= 60,
      current: currentStep === 'scenes',
    },
    {
      id: 'production',
      title: 'Production',
      completed: progress >= 70,
      current: currentStep === 'production',
    },
    {
      id: 'editing',
      title: 'Editing',
      completed: progress >= 80,
      current: currentStep === 'editing',
    },
    {
      id: 'review',
      title: 'Review & Polish',
  ...
  return (
    <div className="space-y-4">
          ...
          {workflowSteps.map((step, index) => (
            <div key={step.id} className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {step.description && (
                  <div className="text-slate-400 text-xs truncate">{step.description}</div>
                )}
              </div>
              ...
              <div className="text-xs">
                {step.completed && (
                  <span className="bg-green-600/20 text-green-400 px-2 py-1 rounded">Done</span>
                )}
                {step.current && !step.completed && (
                  <span className="bg-purple-600/20 text-purple-400 px-2 py-1 rounded">Active</span>
                )}
              </div>
            </div>
          ))}
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        ...
        <div className="bg-slate-800/50 rounded-lg p-3">
          <div className="text-purple-400 text-lg font-bold">{currentStepIndex >= 0 ? 1 : 0}</div>
