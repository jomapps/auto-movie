    ...
    if (message.metadata?.type === 'choice_result') {
      return (
        <div className="bg-purple-600/20 border border-purple-600/30 rounded-lg p-3 mb-2">
          <div className="text-purple-400 text-sm font-medium mb-1">Choice Selected</div>
    ...
    if (message.metadata?.type === 'step_transition') {
      return (
        <div className="bg-blue-600/20 border border-blue-600/30 rounded-lg p-3 mb-2">
          <div className="text-blue-400 text-sm font-medium mb-1">Workflow Progress</div>
          <div className="text-white">{message.content}</div>
        </div>
      )
    }
      ...
      <div className={`max-w-3xl ${isBot ? 'mr-12' : 'ml-12'}`}>
        <div
          className={`rounded-2xl px-6 py-4 ${
            isBot ? 'bg-slate-800 border border-slate-700' : 'bg-purple-600'
          }`}
        >
        ...
        <div className={`mt-2 text-xs text-slate-500 ${isBot ? 'text-left' : 'text-right'}`}>
          ...
          {message.role === 'system' && (
            <span className="ml-2 bg-slate-700 px-2 py-1 rounded text-xs">System</span>
          )}
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        {messages.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">Welcome to AI Movie Chat!</h3>
            <p className="text-slate-400 mb-6 max-w-md mx-auto">
              I&apos;m your AI assistant ready to help guide you through the movie production
              process. Start by telling me about your project or ask any questions.
            </p>
