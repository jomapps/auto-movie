...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    const invalidUserIds = data.userIds.filter(
      (userId: string, index: number) => users[index] === null
    )
    ...
    const existingCollaboratorIds = (project.collaborators || []).map(c =>
      typeof c === 'string' ? c : c.id
    )
    const newCollaboratorIds = data.userIds.filter(
      (userId: string) =>
