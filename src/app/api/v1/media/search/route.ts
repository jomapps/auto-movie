    ...
    if (similarTo) {
      ...
      try {
        ...
        if (!referenceMedia || referenceMedia.project !== projectId) {
          return NextResponse.json(
            { error: 'Reference media not found or not in project' },
            { status: 404 }
          )
        }
        ...
        if (
          referenceMedia.embedding &&
          Array.isArray(referenceMedia.embedding) &&
          referenceMedia.embedding.length > 0
        ) {
          ...
          if (
            referenceMedia.tags &&
            Array.isArray(referenceMedia.tags) &&
            referenceMedia.tags.length > 0
          ) {
...
function calculateSimilarity(reference: Media, candidate: Media): number {
  ...
  const refTags = Array.isArray(reference.tags) ? reference.tags : []
...
function calculateTextRelevance(query: string, media: Media): number {
  ...
  if (media.filename) {
    const filenameWords = media.filename
      .toLowerCase()
      .split(/[^a-z0-9]+/)
