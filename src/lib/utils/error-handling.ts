...
export function handlePayloadError(error: unknown, operation: string): ProjectError {
  ...
  if (error && typeof error === 'object') {
    ...
    // Handle BSON errors (MongoDB ObjectID issues)
    if (errorObj.name === 'BSONError' || errorObj.message?.includes('24 character hex string')) {
      return createProjectError(
        'VALIDATION_ERROR',
        'Invalid data format. Please ensure all required fields are properly filled.',
        { operation, originalError: errorObj.message }
      )
    }

    // Handle PayloadCMS validation errors
    if (errorObj.name === 'ValidationError' || errorObj.errors) {
      return createProjectError('VALIDATION_ERROR', errorObj.message, {
        operation,
  ...
  return createProjectError('PAYLOAD_API_ERROR', `${operation} failed: ${String(error)}`, {
    operation,
  })
}
